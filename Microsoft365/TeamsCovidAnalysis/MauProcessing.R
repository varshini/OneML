...
mau1 <- fread('C:\\Users\\varamase\\Documents\\DataStreams\\TeamsCovidAnalysis\\2020-02_O365MauTimeseries.csv', header = T, sep = ',')
colnames(mau1)[3] <- "TenantId"
mau1$ym <- format(as.Date(mau1$Date, "%m/%d/%Y"), "%Y%m")
mau <- rbind(mau,mau1)

cd <- mau %>% group_by(TenantId, Workload) %>% arrange(ym) %>% mutate(MauTrend=Trending(Mau))
cd <- cd %>% filter(!(ym %in% c("201801", "201802", "201803", "201804", "201805", "201806")))
...
pau1 <- fread('C:\\Users\\varamase\\Documents\\DataStreams\\TeamsCovidAnalysis\\2020-02_O365PauTimeseries.csv', header = T, sep = ',')
colnames(pau1)[3] <- "TenantId"
pau1$ym <- format(as.Date(pau1$Date, "%m/%d/%Y"), "%Y%m")
pau <- rbind(pau,pau1)
...
prev <- mau_pau %>% group_by(TenantId) %>% 
  transmute(ym=CalcPrevDate(ym, -6),
            EXO_MauPrev = EXO_Mau,
            ODB_MauPrev = ODB_Mau,
...
mau_pau_final <- mau_pau_final %>% select(TenantId, ym, Tpid.x, 
                                          EXO_MauPrev, ODB_MauPrev, ProPlus_MauPrev, SPO_MauPrev, Teams_MauPrev, 
                                          ...
                                          ProPlusPaidAvailableUnitsPrev,TenurePrev,
...
mau_pau_final1 <- filter(mau_pau_final, ym %in% c("201901", "201902", "201903", "201904", "201905", "201906"))
mau_pau_final2 <- filter(mau_pau_final, ym %in% c("201907", "201908", "201909", "201910", "201911", "201912"))
mau_pau_final3 <- filter(mau_final, ym %in% c("202001", "202002"))
write.csv(mau_pau_final1, "C:\\Users\\varamase\\Documents\\DataStreams\\TeamsCovidAnalysis\\Training_6months_1.csv", row.names = FALSE)
write.csv(mau_pau_final2, "C:\\Users\\varamase\\Documents\\DataStreams\\TeamsCovidAnalysis\\Training_6months_2.csv", row.names = FALSE)



## SCORING - continue from previous ##

scoring <- filter(prev, ym == "202008")

# Make usage variables 
scoring$EXOUsagePrev <- scoring$EXO_MauPrev/scoring$EXOPaidAvailableUnitsPrev
scoring$ODBUsagePrev <- scoring$ODB_MauPrev/scoring$OD4BPaidAvailableUnitsPrev
scoring$ProPlusUsagePrev <- scoring$ProPlus_MauPrev/scoring$ProPlusPaidAvailableUnitsPrev
scoring$SPOUsagePrev <- scoring$SPO_MauPrev/scoring$SPOPaidAvailableUnitsPrev
scoring$TeamsUsagePrev <- scoring$Teams_MauPrev/scoring$TeamsPaidAvailableUnitsPrev

scoring <- scoring %>% mutate_if(is.numeric, funs(ifelse(is.na(.) | !is.finite(.), 0, .))) %>% mutate_if(is.logical, funs(ifelse(is.na(.) | !is.finite(.), F, .)))

# create tipping points for workloads
scoring <- scoring %>% mutate(ExoTipP = ifelse(EXOUsagePrev >= 0.4, T, F),
                                          CoreAppsTipP = ifelse(ODBUsagePrev >= 0.5, T, F),
                                          OdspTipP = ifelse(ProPlusUsagePrev >= 0.4, T, F), 
                                          TeamsTipP1 = ifelse(SPOUsagePrev >= 0.1, T, F),
                                          TeamsTipP2 = ifelse(TeamsUsagePrev >= 0.4, T, F))
...
scoring_final <- scoring %>% select(TenantId, ym, 
                                          EXO_MauPrev, ODB_MauPrev, ProPlus_MauPrev, SPO_MauPrev, Teams_MauPrev, 
                                          EXO_TrendPrev, ODB_TrendPrev, ProPlus_TrendPrev, SPO_TrendPrev, Teams_TrendPrev,
                                          EXOPaidAvailableUnitsPrev, SPOPaidAvailableUnitsPrev, OD4BPaidAvailableUnitsPrev, TeamsPaidAvailableUnitsPrev, 
                                          ProPlusPaidAvailableUnitsPrev,TenurePrev,
...
colnames(tenant_profile_final)[1] <- "TenantId"
scoring_final <- inner_join(scoring_final, tenant_profile_final, by = "TenantId") 

write.csv(scoring_final, "C:\\Users\\varamase\\Documents\\DataStreams\\TeamsCovidAnalysis\\Scoring_6months.csv", row.names = FALSE)
